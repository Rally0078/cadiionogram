...
class XYPlotCanvas(FigureCanvas):
    ...
    def plot_scatter(self, time_index, heights, x, y, xpow, output_freqs, selected_frequencies, date, site):
        for ax in self.axs:
            ax.clear()
        ...
        for freq in np.unique(selected_frequencies):
            self.ax_range.plot(heights.iloc[np.argwhere(output_freqs == freq).flatten()], marker='s', linewidth=0, markersize=3, label=f"{freq/1e6} MHz")
            self.ax_ew.plot(x.iloc[np.argwhere(output_freqs == freq).flatten()], linewidth=0, marker='s', markersize=3, label=f"{freq/1e6} MHz")
            
            self.ax_ew.set_ylim(-1000, 1000)
            self.ax_ns.plot(y.iloc[np.argwhere(output_freqs == freq).flatten()], linewidth=0, marker='s', markersize=3, label=f"{freq/1e6} MHz")
            self.ax_ns.set_ylim(-1000, 1000)    
        for ax in self.axs:
            ax.set_xlim(time_index[0], time_index[-1])
            ax.set_xlabel(f"Time {site_dict[site].timezone}")
