...
from src.cadiparser.baserawreader import DataReader
...
class MDreader(DataReader):
    ...
    def read_raw_data(self, filename: Path, 
        ...
        cache_output_parent = filename.parent
        ...
        #This class is doing more work than what is intended(only MD file reading, not caching)
        #Caching should be a decorator around this class
        if cache_dir is not None:
            cache_output_parent = cache_dir
        file_exists = False
        if cached:
            file_exists = os.path.exists(f"{cache_output_parent / filename.stem}_{extension}.parquet")
        if cached and file_exists:
            print(f"Reading cached parquet from {cache_output_parent / filename.stem}_{extension}.parquet")
            file_list, metadata, height, frequency, freqs, dop_shifts, complex_signal = ParquetUtils.read_from_parquet(Path(f"{cache_output_parent / filename.stem}_{extension}.parquet"))
        else:
            try:
        ...
        if cached and not file_exists:
            print(f"Writing cached parquet to {cache_output_parent / filename.stem}_{extension}.parquet")
