...
class ScaleRegionValues:
    ...
    def h(self, h_val):
        ...
        if hline is not None and hline in self._ax.lines:
            hline.remove()
        s['hline'] = self._ax.axhline(h_val, color=self._colors[self._current_region]['h'], linewidth=self._line_width, linestyle='--', label=fr"$h'{self._current_region}$ = {h_val:.2f} km")
        s['h'] = h_val
    ...
    def f(self, f_val):
        ...
        if fline is not None and fline in self._ax.lines:
            fline.remove()
        s['fline'] = self._ax.axvline(f_val, color=self._colors[self._current_region]['f'], linewidth=self._line_width, linestyle='--', label=fr"$f_o{self._current_region}$ = {f_val:.2f} MHz")
        s['f'] = f_val
