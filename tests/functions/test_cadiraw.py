...
class TestCADIRaw:
    ...
    def test_read_rawfull(self, test_raw_files_day, test_raw_reader):
        for test_files_single_folder in test_raw_files_day:
            metadata, heights, freqs, dop_shifts, sensors = test_raw_reader.read_raw_data_dir(test_files_single_folder, 'md3')
            assert metadata['extension'] == 'md3'
            assert heights.shape == freqs.shape
            assert heights.shape[0] == sensors.shape[0]
            assert dop_shifts.shape == heights.shape
            metadata, heights, freqs, dop_shifts, sensors = test_raw_reader.read_raw_data_dir(test_files_single_folder, 'md4')
    ...
    def test_mt_reader(self, test_raw_files_day, test_raw_reader):
        for test_files_single_folder in test_raw_files_day:
            metadata, heights, freqs, dop_shifts, sensors = test_raw_reader.read_raw_data_dir(test_files_single_folder, 'md3', multithread=False)
            new_metadata, new_heights, new_freqs, new_dop_shifts, new_sensors = test_raw_reader.read_raw_data_dir(test_files_single_folder, 'md3', multithread=True)
            ...
            metadata, heights, freqs, dop_shifts, sensors = test_raw_reader.read_raw_data_dir(test_files_single_folder, 'md4', multithread=False)
            new_metadata, new_heights, new_freqs, new_dop_shifts, new_sensors = test_raw_reader.read_raw_data_dir(test_files_single_folder, 'md4', multithread=True)
            assert metadata == new_metadata
            assert (heights == new_heights).all()
            assert (freqs == new_freqs).all()
            assert (dop_shifts == new_dop_shifts).all()
