class TestCADIRawIntegration:  
    def test_read_rawfull(self, test_raw_files_day, test_raw_reader):
        for test_files_single_folder in test_raw_files_day:
            for extension in ['md3', 'md4']:
                files_list, metadata, heights, freqs, freq_list, dop_shifts, sensors = test_raw_reader.read_raw_data_dir(test_files_single_folder, extension)
    ...
    def test_mt_reader(self, test_raw_files_day, test_raw_reader):
        for test_files_single_folder in test_raw_files_day:
            for extension in ['md3', 'md4']:
                files_list, metadata, heights, freqs, freq_list, dop_shifts, sensors = test_raw_reader.read_raw_data_dir(test_files_single_folder, extension, multithread=False)
                new_files_list, new_metadata, new_heights, new_freqs, new_freq_list, new_dop_shifts, new_sensors = test_raw_reader.read_raw_data_dir(test_files_single_folder, extension, multithread=True)
