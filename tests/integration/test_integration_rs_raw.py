...
class TestCADIRustRawIntegration:
    ...
    def test_read_raw_rs_badfile(self, test_raw_badfile, test_rust_raw_reader):
        file_list, metadata, heights, frequencies, freq_list, dop_shifts, sensors = test_rust_raw_reader.read_raw_data(test_raw_badfile)
    ...
    def test_py_rs_reader_equality_badfile(self, test_raw_badfile, test_raw_reader, test_rust_raw_reader):
        ...
        file_list_rs, metadata_rs, heights_rs, frequencies_rs, freq_list_rs, dop_shifts_rs, sensors_rs = test_rust_raw_reader.read_raw_data(test_raw_badfile)
        assert metadata == dict(metadata_rs)
        assert (heights == heights_rs).all()
        assert (freq_list == freq_list_rs).all()
        assert (frequencies == frequencies_rs).all()
